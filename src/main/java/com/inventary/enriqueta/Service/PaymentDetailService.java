...
public class PaymentDetailService {
    ...
    public PaymentDetail createPaymentDetail(PaymentDetail paymentDetail) {
        Payment payment = paymentRepository.findById(paymentDetail.getPayment().getId())
                .orElseThrow(() -> new RuntimeException("Payment not found with id " + paymentDetail.getPayment().getId()));
        paymentDetail.setPayment(payment);

        Attorney maleAttorney = attorneyRepository.findById(paymentDetail.getMaleAttorney().getId())
                .orElseThrow(() -> new RuntimeException("Male Attorney not found with id " + paymentDetail.getMaleAttorney().getId()));
        paymentDetail.setMaleAttorney(maleAttorney);

        Attorney femaleAttorney = attorneyRepository.findById(paymentDetail.getFemaleAttorney().getId())
                .orElseThrow(() -> new RuntimeException("Female Attorney not found with id " + paymentDetail.getFemaleAttorney().getId()));
        paymentDetail.setFemaleAttorney(femaleAttorney);

        Student student = studentRepository.findById(paymentDetail.getStudent().getId())
                .orElseThrow(() -> new RuntimeException("Student not found with id " + paymentDetail.getStudent().getId()));
    ...
    public PaymentDetail updatePaymentDetail(String id, PaymentDetail paymentDetailDetails) {
        ...
        Payment payment = paymentRepository.findById(paymentDetailDetails.getPayment().getId())
                .orElseThrow(() -> new RuntimeException("Payment not found with id " + paymentDetailDetails.getPayment().getId()));
        paymentDetail.setPayment(payment);

        Attorney maleAttorney = attorneyRepository.findById(paymentDetailDetails.getMaleAttorney().getId())
                .orElseThrow(() -> new RuntimeException("Male Attorney not found with id " + paymentDetailDetails.getMaleAttorney().getId()));
        paymentDetail.setMaleAttorney(maleAttorney);

        Attorney femaleAttorney = attorneyRepository.findById(paymentDetailDetails.getFemaleAttorney().getId())
                .orElseThrow(() -> new RuntimeException("Female Attorney not found with id " + paymentDetailDetails.getFemaleAttorney().getId()));
        paymentDetail.setFemaleAttorney(femaleAttorney);

        Student student = studentRepository.findById(paymentDetailDetails.getStudent().getId())
